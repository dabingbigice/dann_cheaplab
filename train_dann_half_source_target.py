...
ration = 3
# 目标域文件名列表（无需后缀，若你的文件名带后缀如.png，需保留后缀或后续处理）
TARGET_FILENAMES = {
...
def fit_one_epoch_dann(model_train, model, loss_history, eval_callback, optimizer, epoch,
    ...
    for iteration in range(epoch_step):
        ...
        # 将部分混入源域的目标域标签赋值为1，使其与特征提取器进行对抗
        # 遍历每个样本的文件名，匹配目标域文件名则设为目标域1
        for i, filename in enumerate(imgs_source):
            # 统一文件名格式（去除后缀，避免如"xxx.png"和"xxx"不匹配）
...
class VisualizationTool:
    ...
    def _process_single_image(self, model, name, image, ground_truth, save_dir, domain, idx, total):
        """
        单张图像处理函数 - 用于多线程处理
        """
        try:
            ...
            if domain == 'source':
                ...
                # 真实标签
                label_array = np.array(ground_truth)
            ...
            # 保存对比图像
            plt.tight_layout()
            save_path = os.path.join(save_dir, f"{name}_comparison.jpg")
            plt.savefig(
                save_path,
                dpi=200,
                bbox_inches='tight',
    ...
    def visualize_results(self, model, lines, VOCdevkit_path, save_dir, num_visual=5, domain='source'):
        ...
        print(f"顺序处理 {domain} 领域的可视化结果到 {save_dir}，数量: {min(num_visual, len(lines))}")
...
if __name__ == "__main__":
    ...
    # 数据集路径配置
    source_VOCdevkit_path = '1-2仁'  # 源域数据集路径
    # source_VOCdevkit_path = 'F:\BaiduNetdiskDownload\\1-2仁'  # 源域数据集路径
    target_VOCdevkit_path = 'banli'  # 目标域数据集路径
