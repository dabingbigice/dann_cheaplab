    ...
    def _init_weight(self):
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d):
                ...
                torch.nn.init.constant_(m.b极市ias, 0)
...
class EnhancedLossHistory(LossHistory):
    ...
    def acc_plot(self):
        ...
        try:
            ...
            plt.plot(iters, scipy.signal.savgol_filter(self.acc_details["domain_acc"], num, 3), 'cyan', linestyle='--',
...
# 修改EvalCallback类以支持可视化保存
class EvalCallbackWithVisual(EvalCallback):
    def __init__(self, model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir, cuda,
                 eval_flag=True, period=1, save_dir="eval_visual", num_visual=5):
        super().__init__(model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir, cuda,
                         eval_flag=eval_flag, period=period)
        self.save_dir = save_dir
        self.num_visual = num_visual
        self.val_lines = val_lines
        self.visual_tool = VisualizationTool(num_classes, input_shape)

    # def on_epoch_end(self, epoch, model):
    #     if self.eval_flag and epoch % self.period == 0:
    #         self.model = model
    #         # 创建当前epoch的可视化文件夹
    ...
    #
    #         # 计算mIoU并保存可视化结果
    #         miou = self.get_miou_png(epoch_dir)
    #
    #         # 记录mIoU
    ...
    #
    #         # 可视化验证集结果
    #         self.visual_tool.visualize_results(
    #             model,
    #             self.val_lines,
    #             self.VOCdevkit_path,
    #             epoch_dir,
...
if __name__ == "__main__":
    ...
    # 数据集路径配置
    source_VOCdevkit_path = 'F:\BaiduNetdiskDownload\VOCdevkit'  # 源域数据集路径
    ...
    if local_rank == 0:
        ...
        # 使用源域和目标域的平均比例
        class_ratios = source_class_ratios

        # 计算类别权重（使用中位数频率平衡方法）
        cls_weights = compute_class_weights(class_ratios, method='median_frequency')
    else:
        # 使用默认权重（将在后续广播）
        cls_weights = np.ones([num_classes], np.float32)
    ...
    if True:
        ...
        # ----------------------#
        if local_rank == 0:
            # 使用新的EvalCallbackWithVisual类
            eval_callback = EvalCallbackWithVisual(model, input_shape, num_classes, source_val_lines,
